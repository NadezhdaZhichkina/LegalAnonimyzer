...
def remove_sensitive_data(text):
    patterns = [
    r'(?i)[\t:]\s*[А-ЯЁ][а-яё]+ [А-ЯЁ][а-яё]+ [А-ЯЁ][а-яё]+',  # ФИО после табуляции/двоеточия
    ...
    r'\b\d{6},?\s*город\s+[А-ЯЁа-яё]+,?.*?(д[ом]*\.?\s*\d+[^,\n\r]*)',
    r'(?i)(расчетный счет|р/с|к/с|кор/с|корреспондентский счет)[^\n\r]*',
    r'(?i)\b(ООО|АО|ПАО|ОАО|ЗАО|ИП|ТСЖ|МУП|ГУП|НКО|МКА|СНТ|ПК|КФХ)\s*[«„“"‟”][^«»„“"‟”]*[»”"]',
    r'(?i)\b(публичное акционерное общество|акционерное общество|общество с ограниченной ответственностью)\s+[“"«„“‟”][^«»„“"‟”]+[”"»“”]',
]
    replacements = {}
