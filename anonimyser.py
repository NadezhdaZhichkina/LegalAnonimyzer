...
def remove_sensitive_data(text):
    text = text.replace('\t', ' ').replace('\n', ' ')  # Убираем табуляции и переносы для надёжного поиска

    patterns = [
        r'/[А-ЯЁ][а-яё]+ [А-ЯЁ]\.[А-ЯЁ]\./',  # /Фамилия И.О./
        r'\b[А-ЯЁ][а-яё]+[\s]+[А-ЯЁ][а-яё]+[\s]+[А-ЯЁ][а-яё]+\b',  # ФИО, включая табуляции
        ...
        r'\b[\w\.-]+@[\w\.-]+\.\w+\b',  # Email
        r'(?:(?:8|\+7)[\s\-]?)?\(?\d{3,4}\)?[\s\-]?\d{2,3}[\s\-]?\d{2}[\s\-]?\d{2,4}',  # Телефон
        r'https?://[^\s,]+',  # Ссылки
        r'\b[\w\.-]+\.(ru|com|pdf|org|net)\b',  # Домены
        ...
        r'(?i)\b(публичное|непубличное)?\s*акционерное\s+общество\s*[«"„“‟”][^«»"“”]+[»"“”]',  # ПАО / НАО
        r'\b(ООО|АО|ПАО|ОАО|ЗАО|ИП|ТСЖ|МУП|ГУП|НКО|МКА|СНТ|ПК|КФХ)\s*[«„“"‟”][^«»„“"‟”]*[»”"]',  # Юрлица с кавычками
    ]
