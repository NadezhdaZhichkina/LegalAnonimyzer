...
def remove_sensitive_data(text):
    ...
    patterns = [
        ...
        # Конкретные юридические адреса
        r'\b\d{6}, город [А-ЯЁа-яё\s]+, наб\. [А-ЯЁа-яё\s]+, дом \d+[^,]*',

        # Банковские реквизиты
        r'(?i)\b(расчетный счет|р/с|к/с|кор/с|корреспондентский счет)\b[^\n\r]*',

        # ООО / ПАО / НАО с кавычками
        r'\b(ООО|ПАО|АО|ОАО|ЗАО|ИП)\s+[«"„“‟”][^«»"“”]+[»"“”]',
    ]
