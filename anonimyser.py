...
def remove_sensitive_data(text):
    # Нормализуем табы и переносы, чтобы шаблоны сработали
    text = text.replace('\t', ' ').replace('\n', ' ')

    patterns = [
        # Подписи типа /Фадеев К.В./
        r'/[А-ЯЁ][а-яё]+ [А-ЯЁ]\.[А-ЯЁ]\./',

        # ФИО в формате: Фамилия Имя Отчество (с пробелами или табами)
        ...
        # Адрес с индексом (включая переносы)
        r'\b\d{6},?\s*город\s+[А-ЯЁа-яё\s]+,?\s*[^,]{0,100}д[ом]*\.?\s*\d+[^,]*',

        # Банковские реквизиты
        r'(?i)(расчетный счет|р/с|к/с|кор/с|корреспондентский счет)[^\n\r]*',

        # Названия организаций с кавычками (ПАО, НАО, ООО и др.)
        r'(?i)\b(публичное|непубличное)?\s*акционерное\s+общество\s+[«"„“‟”][^«»"“”]+[»"“”]',
        r'(?i)\b(общество с ограниченной ответственностью|акционерное общество|индивидуальный предприниматель|муниципальное унитарное предприятие|государственное унитарное предприятие|некоммерческая организация|московская коллегия адвокатов|адвокатская контора)\s+[«"„“‟”][^«»"“”]+[»"“”]',
